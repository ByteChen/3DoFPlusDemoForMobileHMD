...
public class Pvr_Controller : MonoBehaviour
{
    ...
    void Awake()
    {
        Pvr_ControllerManager.ControllerThreadStartedCallbackEvent += ThreadStartedSuccess;
        Pvr_ControllerManager.SetControllerAbilityEvent += CheckControllerState;
        Pvr_ControllerManager.ChangeMainControllerCallBackEvent += CheckMainControlller;
    }
    void Start()
    {
    ...
    private void CheckMainControlller(string index)
    {
    ...
    private void ChangeControllerModel(GameObject controller, int dof)
    {
        if (controller != null && !controller.name.Equals("Goblin_Controller"))
        //if (controller != null && !controller.name.Equals("xxxGoblin_Controllerxxx"))
        {
            if (dof == 0)
            {
                controller.transform.Find("dot").localPosition = new Vector3(0, 0, 3.3f);
                controller.transform.Find("dot").localRotation = Quaternion.Euler(0, 0, 0);
                controller.transform.Find("ray_alpha").localPosition = new Vector3(0, 0.016f, 0.8f);
                controller.transform.Find("ray_alpha").localRotation = Quaternion.Euler(90, 0, 0);
                controller.transform.Find("controller/ppcontroller").localRotation = Quaternion.Euler(285, 180, 0);
            }
            else
            {
                controller.transform.Find("dot").localPosition = new Vector3(0, -0.844f, 3.3f);
                controller.transform.Find("dot").localRotation = Quaternion.Euler(15f, 0, 0);
                controller.transform.Find("ray_alpha").localPosition = new Vector3(0, -0.195f, 0.8f);
                controller.transform.Find("ray_alpha").localRotation = Quaternion.Euler(105, 0, 0);
                controller.transform.Find("controller/ppcontroller").localRotation = Quaternion.Euler(270, 180, 0);
            }
        }
    }
    void Update()
